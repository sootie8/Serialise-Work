...
public static class DelegateSupport
{
	...
	public static object FastInvoke(this MethodInfo mi, object target, params object[] parameters)
	{
		...
		if(!_methods.TryGetValue(mi, out getter))
		{
			...
			if (IsSererializableStruct(target))
			{
				try
				{
					var ret = mi.Invoke(target, parameters);
					count ++;
					return ret;
				}
				catch (System.Exception e)
				{
					UnityEngine.Debug.Log(string.Format("Number Of Passes is {0}", count));
					UnityEngine.Debug.Log(string.Format("Target is {0}", target));
					UnityEngine.Debug.Log(string.Format("MI Name is {0}", mi.Name));
					UnityEngine.Debug.Log(string.Format("Mi Is Generic is {0} and {1}", mi.IsGenericMethod, mi.IsGenericMethodDefinition));
					UnityEngine.Debug.Log(string.Format("Mi Declaring Type is {0}", mi.DeclaringType.FullName));
					return null;
				}
			}
			else
	...
	private static bool IsSererializableStruct(object target)
	{
		return (!(target is System.ValueType));
	}
